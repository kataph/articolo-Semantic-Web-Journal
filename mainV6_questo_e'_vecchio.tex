...
\address[A]{Labotory for Applied Ontology (LOA), \orgname{Institute for Cognition Science and Technology (ISTC)},
...
clear example of what we call ontological functions, while the engineering functions are the ones that, as in Pahl and Beitz's work, exist only to be implemented \myComment{solved} by some \methodsName{singular} %in some way 
...
\item \mytext{$x$ is a behavior if and only if there are at least a doer and a flow that participate in $x$}\TODOinline{[SB: se assumiamo che i ruoli sono giocati da entità distinte, come mi sembra implicitamente facciamo, dovremmo aggiungere $y \neq z$[FC: aggiunto]} \eflist
...
\bflist
 ...
 \item \mytext{any ontological function $x$ is specialized by some systemic function $y$ and, beyond ontological functions, it classifies systemic functions only} \TODOinline{
 %[SB: come sopra, ho usato CN invece di DS; poi ho messo la CL ternaria]
 [FC: 
 %ok, però ho fatto uso della relazione di specializzazione, già appositamente introdotta. 
 Nota che tu hai scritto CL(funz.sys., funz.ont., t), ma la specializzazione tra concetti non funziona così (e.g. in Social Roles and [...]), è ok? Inoltre la seconda congiunzione del definiente forse è un po' forte, e.g. esclude un livello intermedio tra funzioni ont. e sys.]*[SB: se capisco resta aperto solo l'ultimo punto (la seconda congiunzione esclude livelli intermedi): ho modificato per dire che se non è ontologica deve essere systemic, vedi se ti torna][FC: ok]}
...
As said, we obtain the types of ontological functions using the variation of some condition before and after the realisation of the function. For example, it is common to speak of `connect' or `change' functions \cite{pahl_engineering_2007}, that is, of functions whose underlying perdurant is such that the number of operands or, respectively, the operand type is modified during the event. Similarly, other ontological  functions-types could be obtained considering variation of other properties, as done in \cite{borgoCapabilitiesCapacitiesFunctionalities2021} and \cite{borgoKnowledgebasedAdaptiveAgents2019}, or specializing existing ontological functions to special cases, e.g. specialize `change' to `change electrical energy into torque'.
In this paper, we do not characterize further the taxonomy of ontological functions, nor discuss the organisation of their taxonomy beyond what presented in \cite{borgoKnowledgebasedAdaptiveAgents2019}. 
...
\eflist
Since the main-function and the $n$ sub-functions roles of a given \methodsName{singular} are univoquely determined, we can represent the with functional symbols. In particular, we will write $\cst{main^m}$, $\cst{sub}^m_1$, \dots, $\cst{sub}^m_n$ to indicate the roles corresponding to the \methodsName{singular} $\cst{m}$, as per Axiom \refax{ax:pre-method} (we omit the functional dependence of $n$ on $\cst{m}$, for ease of notation).
...
\bflist
  ...
  \item \mytext{$x$ is main-function-of a \methodsName{singular} $\cst{m}$ if and only if it is constantly classified by the any of the sub-function roles correspondig to $\cst{m}$} 
\eflist}
...
In this section, we discuss capabilities and capacities, two notions that we briefly introduced at the end of Section~\ref{sec:review}. In particular, we will make use of the concept of  ontological function to distinguish capabilities from capacities (recall that, following the approach taken in ISO 15531-31\cite{jochemISOISO15531312004}, the quantitative viewpoint on what a device do is characterised by capacities, while the qualitative viewpoint by capabilities).
...
The intuition is that capabilities are relational qualities that associate the artifact to ontological functions, so that they must refer to those functions: in our terminology, they are definition-founded\myComment{\TODOinline{[SB: qui c'è "definition-founded", prima c'era "definitional-founded", ricontrollare e unificare][FC: e' accaduto perche' ho applicto la correzione mentre stavi leggendo]}} on ontological functions.  %\footnote{In this context, a heuristic interpretation of definition-dependence at the meta-level could be similar to Definition \myex{ex:Capab}\TODOinline{[SB: nota che questo riferimento è saltato, a cosa ti riferivi?][FC: in realtà, la nota è poco chiara. La rimuovo]}}. 
...
SELECT  ?component ?functionSystemic ?verb ?method
	WHERE { ?functionSystemic ?verb ?method .
		...
		?functionSystemic :function-of ?component }
\end{comment}
Given some ontological function, one can set a query to return the list of \methodsName{plural}, present in the database, that can be implemented through systemic-function specialisation of the input ontological function; or explain what \methodsName{singular}-(sub)types specialize a given \methodsName{singular}, and what systemic-function-types are required by that \methodsName{singular}. 
