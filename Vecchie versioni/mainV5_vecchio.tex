...
\color{red}{QUESTA E' VERSIONE VECCHIA}
}
...
The proposed vocabularies range from Keuneke's short four terms list (toMake, toMantain, toPrevent, and toControl) \cite{keuneke_device_1991}, to larger vocabularies built using complex algorithms, as Kitamura et al.'s \cite{kitamuraFunctionalConceptOntology2002a}. 
...
In any case, to date numerous modelling frameworks have been developed in order to deal with functional and behavioural representation of an engineering systems (e.g., Umeda et al.'s FBS \cite{umedaFunctionBehaviourStructure1990}, Sasajima et al.'s FBRL \cite{sasajimaFBRLFunctionBehavior1995}, Sembugamoorthy and Chandrasekaran's FR \cite{sembugamoorthy1986functional}, Goel et al.'s SBF \cite{goelUseDesignPatterns2004}, Qian and Gero's FBS \cite{qianFunctionBehaviorStructure1996}).
...
clear example of what we call ontological functions, while the engineering functions are the ones that, as in Pahl and Beitz's work, exist only to be implemented \myComment{solved} by some method %in some way 
...
In \DOLCE roles ($\DOLCERole{\cdot}$) are reified and considered as concepts ($\DOLCEConcept{\cdot}$), which themselves form a class subsumed by the class of non-agentive social objects ($\DOLCENASO{\cdot}$):
\bflist
...
\eflist
In this paper, the fact that roles are founded is particularly important, thus, we give the following formalisation, where $\DOLCECLby{\cdot}{\cdot}{\cdot}$ (`classified-by', also called `play-as', if the first argument is a role) is the classification relation between a concept and its instances at a certain time, cf. \cite{masoloSocialRolesTheir2004}. In the formalisation we use some other relations taken from \DOLCE, namely: $\DOLCEConstitutes{\cdot}{\cdot}$ (in \DOLCE it is written $\DOLCEK{\cdot}{\cdot}$) is the  relation holding between some amount of matter and an object when the latter is made of the first;  $\DOLCEOver{\cdot}{\cdot}$ is the standard overlap relation; and $\DOLCEPRE{\cdot}{\cdot}$ is the relation "being present (exist) at time". %, and $\playAs{\cdot}{\cdot}{\cdot}$ is its restriction to roles:
\bflist
...
We also need to capture a different kind of founding relation, that we call \firstTimeKeyWord{definition-founding} ($\foundedDef{\cdot}{\cdot}$). We do not formalize this relation as it requires to discuss how to formally define roles, a topic beyond our concerns in this paper. We will use it with the following informal interpretation: \mytext{$x$ is definition-founded on some entity $y$ if and only if $y$ is used to define $x$}.
...
\myComment{\textcolor{blue}{(non seguo bene la distinzione qui (ma magari e' un problema mio); guardando a d5, uno potrebbe scrivere $founded_{cn}(doctor-role, patient-role)$. Pero' capisco che qst non andrebbe bene. xke'? Potreste fare un esempio in cui $founded_{cn}$ si applica?)}\TODOinline{[se capisco, la sostanza è che "dottore" è definito usando "paziente" ma un'istanza di dottore on richiede l'esistenze di un'istanza di paziente. Forse si potrebbero chiamare: instantiation-founded e definition-founded per sottolineare meglio questa differenza.][FC: come dice SB. Ho sostituito 'instantiation-founded' con 'instance-founded']*[SB: usare instance e definitional non mi piace, userei due sostantivi opp. due aggettivi. Scegli tu. Inoltre $founded_{CN}$ dovrebbe diventare $founded_{Inst}$][FC: messo "instantiation-"][FC:messo anche definition al posto di definitional]}}

Additionally, we will make use of \firstTimeKeyWord{descriptions} to directly talk about definitions of types of entities. In fact, in this paper, descriptions are \myComment{conceptualised as} reifications of definitions, of relations or of types entities. For example, the content of the Italian constitution can be seen as a particular description (an object) that defines several roles, e.g., the Italian Prime Minister. Another example of descriptions are 
%documents that describe how to carry out technical activities, say \myComment{SB: ho tolto l'espressione perché la nozione di documento crea confusione tra oggetto materiale e contenuto}
IKEA assembly instructions. In the latter case, the entities described are perdurants of a certain kind.
In \DOLCE descriptions, $\DOLCEDescription{\cdot}$, are conceptualized as a subcategory of non-agentive social objects:
\bflist
\item[\myax{descriptionSussum}] $ \DOLCEDescription{x} \myfi \DOLCENASO{x} $ 
\eflist

%\myComment{\TODOinline{[Meglio tagliare il pezzo qui sotto (fino a "... and used-by relations"), non aggiunge nulla e rischia di confondere visto che le relazioni DS e US non sono banali quando si parla di concetti][FC: fatto]}}
%\myComment{In addition, though it is not necessary to the development of our theory, descriptions can be used to define the definition-founding relation.
%In fact, if one introduces two relations between entities and descriptions: $\DOLCEDefinedBy{\cdot}{\cdot}$, defined-by, to state that an entity is defined by a description; and $\DOLCEUsedBy{\cdot}{\cdot}$, used-by, to state that a description makes use of an entity\footnote{The interested reader can consult e.g. \cite{masoloSocialRolesTheir2004} for a discussion on descriptions.}, then, these two relations can be used to state 
%\bflist
...
%\item \mytext{$x$ is definition-founded on $y$ if and only if there exists a description, say $z$, that defines $x$ and makes use of $y$} \textcolor{blue}{penso che aiuterebbe i lettori scrivere CN(x) e DS(y)// non ho capito se vale $founded_{def}(x,y) \to founded(x,y)$}
%\eflist
%The definition above is just a clarification of the meaning of definition-founding and descriptions, and, for the rest, we will not use the defined-by and used-by relations. }
...
\item \mytext{A concept $x$ specializes a concept $y$ if and only if all instances of $x$ are also instances of $y$} %\textcolor{blue}{(as above, e' necessario il quant. esistenziale?)}\TODOinline{[il quantificatore esistenziale serve]}
\eflist
For example, the role of the Italian Prime Minister specializes the role of Prime Minister.
This notion of specialization is admittedly weak. One would like to add a modal characterization: definition \refdf{def:conceptSussum} should hold in all possible worlds. This problem applies to other definitions we introduce in this paper and in some special cases we will discuss this issue. The problem is not ontological but related to the limitations of first-order logic. Without discussing logical technicalities, in this paper we will make use of these characterizations assuming that, in suitable systems, e.g. first-order modal logic, a suitable modal formula is substituted.
...
% 2) ho introdotto la relazione "play";[FC:ok-->rimossa per via di cambiamenti: solo menzionata]\\ 
% 3) CL e play hanno bisogno del tempo, sono ternarie;[FC:fatto, allora idem per la partecipazione e per la parte-intero, fatto]\\
...
In this section we propose a framework to characterize how behaviour and function of engineering artifacts can be understood to make sense of the distinctions used by engineers in, e.g., process, product, or early system design planning. Within the following section, we will expand this view to capability and capacity.
...
\end{comment}
The mereology mentioned above is built as usual from the temporalized parthood relation of \DOLCE,  $\DOLCEPart{\cdot}{\cdot}{\cdot}$. We also introduce a non-temporalized relation, see \refdf{def:partConstant}. Axiom \refax{ax:part-present} and the class $\DOLCEPhysObj{\cdot}$, the category of physical objects, are taken from \DOLCE.
\bflist
...
\item[\mydf{partConstant}] $ \DOLCEPartBin{x}{y}  \myiff \exists t (\DOLCEPRE{x}{t}) \land \forall t (\DOLCEPRE{x}{t} \myfi \DOLCEPart{x}{y}{t})$
...
In \DOLCE one models the interaction as a perdurant. How to ontologically understand behavior is more tricky. 
...
Additionally, there is the external-internal axis, that is, the behavior \myComment{(or the behavioral description)} of an artifact may refer only to characteristics of the artifact itself, or it may need to refer to some external entities.
...
Following common practice in the literature, in this paper we take behaviors to form a subclass of \DOLCE perdurants. This class is partially characterized by the fact that they give rise to two `processual'\footnote{The term \quotes{processual role} does not refer to \DOLCE processes. It is taken from Loebe which uses it relatively to the GFO approach \cite{loebeAbstractVsSocial2007}.} roles in the sense of \cite{loebeAbstractVsSocial2007}: an active one, called \firstTimeKeyWord{doer}, and a passive one, called \firstTimeKeyWord{operand} or \firstTimeKeyWord{flow} \cite{pahl_engineering_2007}. 
...
Such behaviors, whose class we write as $\BehaviourConcrete{\cdot}$, are external behaviors, since the flow is an entity external to the behaving artifact (the doer).
...
\item \mytext{$x$ is a behavior if and only if there are at least a doer and a flow that participate in $x$} \eflist
...
\item[\myax{goalSubsum}] $ \Goal{x} \myfi \DOLCEConcept{x} \land \forall y,t (\DOLCECLby{y}{x}{t} \to \DOLCEStative{y})$
  \item \mytext{a goal is a concept that classifies stative perdurants only}%\TODOinline{[SB: ho esteso l'assioma, controlla][FC: se in DOLCE è ok che la anche le descrizioni, e non solo i concetti, classifichino, allora è ok]*[SB: hai ragione, stavo usando description come sottoclasse di concept ma non è coerente con l'articolo sui ruoli. Se un goal classifica states, seguendo l'articolo sui ruoli un goal è un concetto (a cui sarà associata una descrizione che però a questo punto non ci interessa). Ho cambiato il testo e usato CN, controlla)][FC: dovrebbe esser ok]}
\eflist
Thus, goals may correspond to expressions \quotes{the temperature at the port B of the heat exchanger is between 80 and 110 Celsius degrees} and \quotes{the buzzer is clapping with a frequency of at least 10kHz}, which are state classifiers.
%, if these states-descriptions have been selected as desired\myComment{\textcolor{blue}{(ES: as desired realisation of goals (?))[SB: non abbiamo caratterizzato quando una descrizione è un goal, per me esce dallo scope di questo lavoro]}}.
...
\myComment{\item[\mydf{functionOf}] $ \FunctionSysOf{x}{y} \myiff (\DOLCERole{x} \land \exists b,t  (\DOLCECLby{b}{x}{t}) \land 
  \exists z,g ((\System{z} \land \DOLCEPartBin{y}{z}  \\ \land \goalOf{g}{z})  \land 
    \forall b,t ((\DOLCECLby{b}{x}{t} \myfi (\BehaviourConcrete{b} \land 
      ...
      \TODOinline{\mbox{[SB: constant part è forse troppo forte?]}}
  ...
  \TODOinline{[SB: constant part è forse troppo forte?][FC: in effetti è un rimasuglio di quando non c'era  l'argomento temporale. Adesso dovrebbe essere ok]}
  \item \mytext{$x$ is a systemic function of $y$ if and only if $x$ is a role and there exist a system $z$ and a goal $g$ for $z$ such that $x$ is satisfied only by behaviors which have $y$ as doer and causally contribute to achieve $g$, whenever $y$ is  part of $z$}
...
\TODOinline{[SB: a partire da qui ho fatto vari cambiamenti importanti]}
...
%  \item[\myax{functionAbstr}] $ \FunctionAbs{x} \myfi (\DOLCEDescription{x} \land \exists y \DOLCECLbyBinary{y}{x} \land \forall y (\DOLCECLbyBinary{y}{x} \to \FunctionSys{y}) $
%  \item \mytext{any ontological function is a description that classifies systemic functions only}
 ...
 $ \FunctionAbs{x} \myfi (\exists y ( \DOLCEConceptSubsum{y}{x}) \land \forall y (\DOLCEConceptSubsum{y}{x} \myfi \FunctionSys{y}) $
 \item \mytext{any ontological function is a concept that classifies systemic functions only} \TODOinline{[SB: come sopra, ho usato CN invece di DS; poi ho messo la CL ternaria][FC: ok, però ho fatto uso della relazione di specializzazione, già appositamente introdotta. Nota che tu hai scritto CL(funz.sys., funz.ont., t), ma la specializzazione tra concetti non funziona così (e.g. in Social Roles and [...]), è ok?]}
%\item[\myax{functionAbstr}] $ \FunctionSys{x} \myfi \exists y (\FunctionAbs{y} \land \DOLCECLbyBinary{x}{y} \land \foundedROLE{x}{y})$
%\item[\myax{functionAbstr}] $ \FunctionAbs{x} \myfi \exists y (\FunctionSys{y} \land \DOLCEConceptSubsum{y}{x}) \land (\specificallyDependsOn{x}{y} \myfi \neg \System{y}) $
\eflist
...
Both electrical motors perform the same ontological function of \quotes{converting} (electrical energy into mechanical energy), but they also perform two different systemic functions %that 
specializing the ontological function in the context of the lathe: \quotes{converting (electrical energy into mechanical energy) to rotate the spindle}, and \quotes{converting (electrical energy into mechanical energy) to translate the spindle}, respectively.
The intuition is that we can group systemic functions together through common characteristics, abstracting from specific systems or from their occurrences in different parts of the same system.

Such ontologically functions reflect a perdurant taxonomy based on functional grounds, meaning that a given taxonomy of perdurants corresponds to a taxonomy of functions as given in \cite{borgoKnowledgebasedAdaptiveAgents2019}.
%, by stating that functions of some type must be played only by occurrents of the corresponding type. 
As said, we obtain the types of ontological functions using the variation of some condition before and after the realization of the function. For example, it is common to speak of `connect' or `change' functions \cite{pahl_engineering_2007}, that is, of functions whose underlying perdurant is such that the number of operands or, respectively, the operand type is modified during the event. Similarly, other ontological  functions-types could be obtained considering variation of other properties, as done in \cite{borgoCapabilitiesCapacitiesFunctionalities2021} and \cite{borgoKnowledgebasedAdaptiveAgents2019}, or specializing existing ontological functions to special cases, e.g. specialize `change' to `change electrical energy into torque'.
In this paper, we do not characterize further the taxonomy ontological functions, nor discuss the organization of their taxonomy. 
...
First, there are \quotes{general} functions that engineer use when need to, say, describe information about or collect information from different systems. For example, Collins \cite{collinsFailureExperienceMatrixUseful1976}, when collecting and analyzing failure experience data, speaks of \quotes{elemental mechanical functions}, which are application-independent characterisations of \quotes{basic} functions. %\quotes{distinctive generic characterization of the basic function of a machine part without reference to the specific application for which it is used} 
...
\end{figure}
In our terminology, we could say that tags identify  roles that components play in a system. Necessarily, these role are of functional nature, for each component in a engineering system has a role in the system function, thus, tags, or, at least, the teleological content they carry, could be formalized by systemic functions. %Indeed, tags should carry also information about, say, the structural links of the underlying component with the remaining components of the systems. 
\TODOinline{[SB: manca un risultato conclusivo che giustifichi perché ci fermiamo qui. bisogna dare almeno qualche indicazione di quali concetti di funzione usati in ingegneria cadono sotto questo approccio (nel senso che possono essere considerate specializzazioni di queste definizioni)][ Ho provato a inserire un paragrafo argomentando che le "generally valid functions" di Pahl e Beitz assomigliano alle funz.ont. e che i tag/sigle/functional location assomigliano alle fun.sys.]}
%\textcolor{blue}{(se capisco, le ontological function sono ruoli giocati dai perdurant. E qst perdurant sono i behavior degli artefatti/sistemi che hanno come scopo la realizzazione di un certo goal.  Per aiutare i lettori, sottolinerei nel testo che la nozione di ontological function e' un ruolo/concetto (forse ho perso qst informazione))(FC: è sotto la definizione di funz. sys., ma per sicurezza ho aggiunto anche all'inizio di sottosezione)}
...
Inspired by the work of Kitamura et al. on `ways of functional achievement' \cite{kitamuraOntologicalModelDevice2006, kitamuraOntologybasedDescriptionFunctional2003}, we conceptualize engineering methods as descriptions of the knowledge that engineers share about ways of implementing functions through functional decomposition:
\bflist
  \item[\myax{methodSubs}] $ \Method{x} \myfi \DOLCEDescription{x}$ \TODOinline{[SB: dopo le variazioni questo è l'unico posto dove parliamo di descrizioni e non mi sembra neppure rilevante. visto che qui il metodo è solo informazione per la functional decomposition, potremmo chiamarlo wayOfAchievement e usarlo come relazione: data una main-function f, $wayOfAchievement(f,f_i)$ vale se $f_i$ è una sottofunzione]} 
%   \land \DOLCEDescription{x}  \myfi \DOLCENASO{x}$
\eflist
Formally, such descriptions can be understood as reifications of functional decomposition relations.\TODOinline{[SB: l'uso del termine description non va bene, nell'articolo sui ruoli le description sono entità complesse, quindi non possono essere una reificazione, suggerisco di togliere tutto e usare $wayOfAchievement$]}
Precisely, we introduce binary relations $\mainFunction{\cdot}{\cdot}$ and $\subFunction{\cdot}{\cdot}$ \TODOinline{[SB: qual è il significato di queste relazioni? come fa un lettore a capire?]}, such that the following definition schema holds:
\bflist
  \item[\mydf{method}] $\decom(\cst{f};\cst{f}_1,\cst{f}_2,\dots,\cst{f}_n) \myiff \\ ( \FunctionSys{\cst{f}} \land \FunctionSys{\cst{f}_1} \land \dots \land \FunctionSys{\cst{f}_n} \land \\ \exists \cst{m} (\Method{\cst{m}} \land \mainFunction{\cst{f}}{\cst{m}} \land \subFunction{\cst{f_1}}{\cst{m}} \land \ldots \land \subFunction{\cst{f_n}}{\cst{m}}) 
 $
 \item \mytext{$f$ is decomposed in $f_1,\ldots,f_n$ if and only if they are all systemic functions and there is a method to execute all of them}\TODOinline{[SB: non mi è chiaro perché lo stesso $m$ vale per tutte, l'intuizione mia è che $m$ debba essere decomponibile e specifico per ogni funzione. l'esempio del motore elettrico (import electric energy, transform, export torque) a me fa pensare che che ogni sotto funzione ha il suo metodo][FC: non ti seguo tanto qui, a me sembra chiaro che, per esempio, nell'approccio di Riichiro il metodo/way-of-achievement è usato come un "label" di una decomposizione funzionale. Il modo più pacifico di metterlo in simboli è una reificazione individuale della relazione di decomposizione, come ho fatto sopra. Vedi e.g. le slide di RIichiro, le prime che ci ha passato, con le mie annotazioni]
 \includegraphics[width=\linewidth]{decomposizione.PNG}*[SB: quindi il metodo qui è solo un collante tra funzione e sottofunzione, non fornisce la procedure di come eseguire la funzione. questo va detto. io avevo in mente i metodi in ingegneria che sono delle procedure.}
\eflist
In the previous definition, main-functions and sub-functions should be understood as relational roles in the context of a decomposition (for sake of brevity, we implement main- and sub-functions as binary relations instead of role-concepts\TODOinline{[SB: non è un buon argomento. visto che abbiamo usato i ruoli fino ad ora e introdotto tutte le forme di founding che servono, continuiamo a modellare usando i ruoli dove servono]}\footnote{An extended implementation would say that a method-description is the context of a main-function role and of a number, say $n$, of sub-function roles. Then, the decomposed function $\cst{f}$ is in relation $\DOLCECLby{\cdot}{\cdot}{\cdot}$ with the main-function role, and the $n$ decomposing functions $\cst{f_i}$ are in relation $\DOLCECLby{\cdot}{\cdot}{\cdot}$ with the corresponding sub-function roles.}).

The advantage of this approach is manyfold: it makes  possible to organize the method-types within sumbsumption taxonomies, for example, `spot welding' is a specialisation of the method `welding', which itself is an implementation method of the function `to join'; %\footnote{The taxonomy of methods also prevents Vermaas'proof from applying \cite{vermaasFormalImpossibilityAnalysing2013}.\TODOinline{[SB: non mi sembra rilevante, togliamo questa nota]}}; 
and to describe the properties of the methods, for instance the working principle, say Kirchhoff's law for a `voltage divider' method. %, similarly to what has been done by Kitamura's team in  \cite{kitamuraOntologybasedDescriptionFunctional2003}.
...
\bflist
  ...
  \item[] \mytext{engineering functions and main-functions coincide} %\TODOinline{[SB: ho tolto "role defined by a method-description, and, therefore, contextualized by a functional decomposition relation" perché nella lettura in linguaggio naturale delle formule non si deve includere niente altro, neppure spiegazioni od osservazioni][FC:ok]}
\eflist
...
For instance, in the lathe example discussed above, it could be that the systemic functions of the two motors both are implemented through the method of, say, `three-phase electric motor', and, therefore, play the role of engineering functions. In this case, the functional decomposition  entailed by the method includes sub-functions roles for, say, `supply electrical energy' (one per each phase), `drive the motor', and `output mechanical energy'. 
...
In this section, we discuss capabilities and capacities (refer to the end of Section \ref{sec:review} for a very brief intro on these two terms). In particular, we will make use of the ontological-function concept, defined in the previous section, to distinguish capabilities from capacities.

Each technical artifact has relevant characteristics, which are part of its physical make-up, and are such that they endow the artifact with the ability of being able to do certain things. 
For example, an individual pump could be built in such a way that it is able to, say, pump %hydraulic 
...
Consequently, both the flow rate (a capacity) and the pumping (a capability) of the pump in the previous example are introduced in the %discourse domain
domain of discourse as individual qualities.
...
Coming back to Gero and Qian's example of the water tap mentioned in Section \ref{sec:DOLCE}, which is similar to the one of the pump, we could say that the faucet has the capability of delivering water when requested, which is founded on its flow rate capacity, which is itself founded 
%of 
on its diameter quality (Gero and Qian say that the flow rate is \quotes{controlled} by the diameter \cite{qianFunctionBehaviorStructure1996}).
...
In contrast, a characterisation of capabilities should touch also functions, since capabilities are inextricably intertwined to functional aspects. In order to do this, we first try to clarify the nature of capabilities as following:
...
Thus, as a simplifying assumption on the formal language, we shall consider that the introduction of capabilities allows us not to use modalities. 
Then, since the newly introduced capabilities are specifically dependent on their bearers, and each capability of a certain kind is unique to its bearer, we treat them as individual qualities.\footnote{An alternative is to model them as disposition, see for instance \cite{sarkarOntologyModelProcess2019}.} 
...
\item[] \mytext{A relational quality is a capability if and only if is founded on some capacity and is definition-founded on some ontological function}
\eflist
...
The intuition is that capabilities are relational qualities that are needed to carry out an ontological function, so that they refer to that function: in our terminology, they are definition-founded\myComment{\TODOinline{[SB: qui c'è "definition-founded", prima c'era "definitional-founded", ricontrollare e unificare][FC: e' accaduto perche' ho applicto la correzione mentre stavi leggendo]}} on an ontological function.  %\footnote{In this context, a heuristic interpretation of definition-dependence at the meta-level could be similar to Definition \myex{ex:Capab}\TODOinline{[SB: nota che questo riferimento è saltato, a cosa ti riferivi?][FC: in realtà, la nota è poco chiara. La rimuovo]}}. 
Note that they are not \refdf{def:founding}-founded, since an artifact could have a capability to function without actually functioning. Additionally, the function must be ontological and not systemic, because, otherwise, the bearing object would be associated \textit{a priori} with some given system. 

Even though we do not fully characterized capacities, \refax{ax:capacPartialDef} and \refdf{def:capability} have the advantage of explaining
\begin{itemize}
...
\end{itemize}
The quantitative-qualitative difference between capacities and capabilities anticipated at the end of Section \ref{sec:review} is still unexplained, but, notice that while a flow-rate capability takes as values positive real numbers with, say, m$^3$/s as unit of measure, the value space of the corresponding pumping capability is quite more complex and difficult to describe. This  may be an argument against modelling capabilities as \DOLCE-qualities, but it also reflects the quantitative-qualitative duality that emerges in domain experts' speech. %\TODOinline{[SB: questa sezione sulle capacities/capabilities è migliorata molto ora :)][FC::)]}

\medskip
Finally, to recap some of the concepts introduced in the last paragraphs, we conclude this section with another example.\myComment{\footnote{Notice again the similarity with the work of Kitamura, Sasajima, Mizoguchi et al.\cite{kitamuraOntologybasedDescriptionFunctional2003}.}} Suppose that a company handling swimming pools must empty some of its pools for maintenance. This imposes a goal, say `the pools are empty', that must be carried out through some device able to realize a `to empty' (ontological) function, specialized to a systemic function in the context of the swimming pool system.
Such a function can be realized only by some artifact that has a corresponding capability. In this case, the function would probably be implemented through some fluid-emptying-method, that is, through the shared knowledge of the way that a recipient can be emptied, and its fluid content disposed, by pressurizing the fluid, guiding it through a path, and finally storing it.
...
The first order logic theory developed in the previous sections can be converted to \OWL language as is, with the exception of the expressions where ternary relations are used.
For example, the definition of a systemic function \refdf{def:functionOf}-\refdf{def:function}, the instantiation-founding definition \refdf{def:founding}, and the engineering method schema \refdf{def:method} cannot be expressed in \OWL. 
...
SELECT  ?component
	WHERE { 
	    ?capability :definition-founded-on this:<the given function> .
...
For example, the following query takes a given system as inputs and returns all systemic functions involved in a method, with their role (main-function or sub-function) and underlying component:
\begin{verbatim}
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
PREFIX : <http://www.co-ode.org/ontologies/ont.owl#>
...
SELECT  ?component ?functionSystemic ?verb ?method
	WHERE { ?functionSystemic ?verb ?method .
		...
		?functionSystemic :function-of ?component }
\end{comment}
Given some ontological function, one can set a query to return a list of methods that can be implemented through systemic-function specialisation of the input ontological function; or explain what method-(sub)types specialize a given method, and what systemic-function-types are required by that method. 
...
Additionally, for the sake of example, it is populated with a few individuals.
Moreover, even though the \DOLCE ontology is primarily a first-order logic theory, \OWL-adapted versions exists, such as \DOLCE-lite or \DOLCE-Ultralite\footnote{Both openly available, e.g., at \url{http://www.loa.istc.cnr.it/dolce/overview.html}.}. 
Any of these could be used to align the ontology developed in this paper with \DOLCE. In our case, we used the \OWL version of \DOLCE that has been recently submitted for inclusion in the ISO 21838 standard\footnote{\url{https://www.iso.org/standard/71954.html}.}, which can be found on GitHub\footnote{\url{https://github.com/kataph/function-method-ontology.git}.}, together with the ontology described in this section.
