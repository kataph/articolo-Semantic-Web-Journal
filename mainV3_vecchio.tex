...
In particular, it is shown how functions can be distinguished from implementation methods and how functions can differentiate between capabilities and capacities of a product. \TODO{is it though?}
\end{abstract}
...
\eflist
In this paper, the fact that roles are founded is particularly important and we give the following formalisation, where $\DOLCECLby{\cdot}{\cdot}{\cdot}$ is the temporalized classification relation between a concept and its instances, cf. \cite{masoloSocialRolesTheir2004}, and $\playAs{\cdot}{\cdot}{\cdot}$ its restriction to roles:
\bflist
...
\item[\mydf{founding}] $ \foundedROLE{x}{y} \myiff (\DOLCERole{x} \land \exists z,t (\playAs{z}{x}{t}) \land \forall z,t (\playAs{z}{x}{t} \myfi \exists w (\founded{z}{w} \land \DOLCECLby{w}{y}{t})))$
...
Additionally, since roles, as well as concepts, can be seen as reified classes, there exists a specialisation relation between roles, that we write as $\DOLCEConceptSubsum{\cdot}{\cdot}$:
\bflist
\item[\mydf{conceptSussum}] $ \DOLCEConceptSubsum{x}{y} \myiff (\DOLCEConcept{x} \land \DOLCEConcept{y} \land \forall z,t (\DOLCECLby{z}{x}{t} \myfi \DOLCECLby{z}{y}{t}) )$\footnote{Note that, technically, \refdf{def:conceptSussum} entails that any empty concept specialises any other concept. This is not important, since we do not care about empty concepts.}
\eflist
...
When \myComment{Whenever<-no: altrimenti sembra definizione} this happens, that is, whenever a capability is founded on a relational quality, we call that quality \firstTimeKeyWord{capacity}, analogously to the approach in \cite{borgoCapabilitiesCapacitiesFunctionalities2021}. 
...
screenshot tipo e.g. \ref{fig:screen_query} \ref{fig:screen_entities}; %e work through a tutorial example per mostrare inferenze utili. Concludere con 
