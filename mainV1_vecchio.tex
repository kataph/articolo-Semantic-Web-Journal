...
\bflist
\item[\mydf{founding}] $ \founded{x}{y} \myiff (\DOLCERole{x} \land \forall z (\playAs{z}{x} \myfi \exists w (\specificallyDependsOn{z}{w} \land \external{z}{w} \land \DOLCECLby{w}{y})))$
\item[] \mytext{$x$ is founded in $y$ if and only if $x$ is a role and given any $z$ that plays $x$, then $z$ depends on an instance of $y$ to which $z$ is external}

%where $Y$ is a class (or a reified class, as a role) and $\DOLCECLby{\cdot}{\cdot}$ (\quotes{classified by}) is the instantiation relation between a reified class and its instances (the inverse relation, in the case of roles, is usually called \quotes{play as} -- $\playAs{\cdot}{\cdot}$),
...
\item[\myax{roleFounding}] $ \DOLCERole{x} \myfi \exists y \founded{x}{y} $ 
\item[] \mytext{if $x$ is a role, then there is a $y$ on which it is founded} cfr. \refax{ax:relationalQtPartialDef}.
\eflist 

\TODO{alcuni punti generali:\\
1) evitiamo di quantificare su classi;\\ 
2) ho introdotto la relazione "play";\\ 
3) CL e play hanno bisogno del tempo, sono ternarie;\\
4) evita punti, virgola etc. a fine delle formule;\\ 
5) per ogni formula inserisci tra parentesi come leggerla in ling. naturale (come ho fatto nelle f.le in questa sez.)}
\textcolor{blue}{ES: secondo me conviene fare qualche esempio a proposito della nozione di 'foundation'per i ruoli. e' utile tenere a mente che la rivista non ha il flavour concettuale di riviste come Applied Ontology o conferenze tipo FOIS; per cui meglio `accompagnare' il lettore in alcuni passaggi}
...
Whenever a capability is based on some other quality of a technical artifact, that is, when each realisation of the capability depends on the other quality, we say that it is \firstTimeKeyWord{\foundedTerm{passive}} on that quality.
...
So that, in the pumping example, the flow rate capacity partially quantifies the pumping capability.  \textcolor{blue}{ES: forse il termine `quantify' crea confusione con la nozione logica di quantificazione. Capisco la frase perche' so cosa si vuole dire ma non so quanto sia chiara ad un lettore esterno (vedi commento sotto)}
...
%Analogously to the work of Masolo \& al. \cite{masoloSocialRolesTheir2004}, 
...
%\footnote{Masolo \& al. use descriptions in order to define concepts, in particular, they say that roles are those concepts that are antirigid and whose description make use of another, external, concept.
%In their work \quotes{external} means that for any instance of the original concept, there exists an instance of the external concept, that is neither part-of nor containig the first instance.}.\TODO{è già stato detto}
In our case, the agent community is formed by engineers sharing common knowledge, %and the roles that have the method-description as context are main-function and sub-function
