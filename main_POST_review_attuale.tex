...
\bflist
\item $\FunctionSys{x}\land\DOLCEPRE{x}{t} \myfi $
\item $\exists b\  \DOLCECLby{b}{x}{t} \myfi $ usando al definizione d8,d9
\item $\exists y,g,z\  \participateAsDoer{y}{b}{t} \land \causallyContr{b}{g} \land \goalOf{g}{z} \myfi$ (chiaramente se il comportamento di y contribuisce al goal g di un sistema z, il systema z è presente al tempo t)
\item $\DOLCEPRE{z}{t}$ 
\eflist
quindi x depends on z, inoltre z è esterno a x per l'ipotesi espressa prima della formula "assume that systemic functions are external to systems", quindi, per definizione x founded-on z
