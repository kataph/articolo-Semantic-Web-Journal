...
\bflist
  \item[\mydf{ontoDef2}] \mytext{A concept is a \ontoFunc{fullSingular} if and only if it classifies exactly those events consisting in a transition from an initial state to a desired final state, such that the difference (if any) between the two states is characterized in terms of a formula expressed in the language of a reference ontology.}
\eflist
...
Since a \qquotes{a formula expressed in the language of a reference ontology} is not writeable using first-order logic in a natural way, we cannot give an (intrinsic) first-logical definition of \ontoFunc{fullPlural}. 
...
\bflist
  ...
  \item[\myCQ{cq4}] How to fruitfully carry out the funcitonal decomposition of a system?
