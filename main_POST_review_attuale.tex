...
The ensuing work and documentation can be daunting, especially in large progects. Therefore, attempts have been made to standardize \cite{alrumaihDomainOntologyRequirements2020}, trace \cite{holterScopeDetectionTextual2021}, digitalize, and automatize requirement pipelines using ontologies. 
...
Other approaches discuss also how requirements should be written, so that their quality, shareability, and even automatic verification agains a design model can be assured \TODO{[cit]}.
In the latter case, requirements are usually interpreted as assertion (constraints) about an engineering artifact that can be expressed (at least some of them) in a formal language. 
For example, a requirement translated directly from an client's request, could be that the product, say a table, \qquotes{must weight as little as possible}, then engineers could precise this by stating 
\bflist
  \item[\myex{req1}] $ weight(table\_top) \leq 5kg \land weight(i_{th}\_table\_leg) \leq 2kg$, for $i = 1,2,3,4$. 
\eflist
Therefore, if one also design products using the same formal language, the products can be checked against the set of constraints, to verify that those are satified by the products \TODO{[cit]}.
One difficulty of this approach is that some requirements are more difficult to express than other.
Requirements such as \refex{ex:req1} are easy to express, as they refer to physical properties of the products (they are sometime called \qquotes{physical requirements} \cite{jinxinlinRequirementOntologyEngineering1996}).
On the other hand, requirements linked to products performance or functions are more difficult to express.
...
\bflist
  \item[\myex{req5}] $ desk\_spot\_lamp(p) \myfi \exists f,c,v( illuminating\_capability(f) \land \DOLCEQualityDirect{f}{p} \land illumination\_area(c) \land \\ \founded{f}{c} \land \DOLCEQualeDirect{c}{v} \land \DOLCEPartBin{v}{(-\infty,0.5]}$
  \item[\myex{req6}] $ base(p) \myfi \exists fo,fs (provide\_support(fo) \land \DOLCECLbyBinary{fs}{fo} \land \FunctionSysOf{fs}{p})$
\eflist
Notice that \refex{ex:req5} and \refex{ex:req6} can be readily formalized in \OWL, so that they be part of an \OWL knowledge base containing all the requirement that can be expressed in a similar way. 
Then, if engineers build during design a model of the product, using the same language, they just need to import the model in the knowledge base with the requirements: the product model satisfies the requirements if and only if the ensuing ontology is consistent.  
