...
For instance, walking may be considered a simple action, but, in order to describe such a process, one needs to \qquotes{project} the waking onto some limited viewpoints, such as the variation of the distance of the feet from the floor, the plane trajectory of the center of mass the body, the changing contact forces between the feet and the floor, as well as those developing into the leg joints, and so on. 
...
In practice, given an upper ontology, one can use its classes and relations to establish corresponding types of \ontoFunc{fullPlural}.
...
\begin{itemize}
  ...
  In general, though, if one actually wishes to classify a transformation that cannot be described by comparison of two states, then the method employed in the previous formulas is not sufficient.
  ...
  This, together with the fact that we admit also \ontoFunc{fullPlural} defined by the absence of some type of change (e.g., \textit{support}), makes so that many events would be classified by several (possibly even all) \ontoFunc{fullPlural} at once.
  ...
  To recognize what aspect is the most relevant in a given context is a task for the engineer or technician, which, using their contextual intelligence and creativity, determine what aspects to focus on.
...
\bflist
  \item[\mydf{ontoDef2}] \mytext{A concept is a \ontoFunc{fullSingular} if and only if it classifies exactly those events consisting in a transition from an initial state to a desired final state, such that the difference (or the absence thereof) between the two states is characterized in terms of a formula expressed in the language of a reference ontology.}
\eflist
(Of course, the formulas that engineers actually use to express ontological functions are not arbitrary, as in \refdf{def:ontoDef2}, but, typically, belong to a small set of cases).
Since \qquotes{a formula expressed in the language of a reference ontology} is not something that can be written in a natural way using first-order logic, we cannot give an intrinsic first-logical definition of \ontoFunc{fullPlural}. 
...
The intuition is that we can group systemic functions together through common characteristics, as we have attempted in the definitions \refdf{def:connect} to \refdf{def:changeQualityValue}, abstracting from specific systems or from their occurrences in different parts of the same system.
...
Moreover, we will discuss how our ontology answers the research questions \refRQ{RQ:RQ1}-\refRQ{RQ:RQ1.2}, as well as some competency questions that will serve to demonstrate the expressibility potential of the ontology.
Finally, we will also discuss some possible application scenarios, though we highlight that the ontology main goals are the research questions \refRQ{RQ:RQ1}-\refRQ{RQ:RQ1.2}, while we delegate the development of application-level ontologies in relevant domains to further work.
...
\begin{itemize}
  ...
  \item \textbf{Answer to \refRQ{RQ:RQ1.1}.} Systemic functions can be decomposed in the way discussed in Subsection \ref{subsec:fun-dec}, in such a way that an engineering method describes how a decomposition must happen. 
...
\end{itemize} 
Moreover, since the formal ontology answers these research questions, it is also able to answer related competency questions, which can be of interest in applications.
...
Additionally, many do not need our ontological analysis of functions and related concepts at all (e.g., \qquotes{what is the value of a certain property of the given component}).
% \bflist
...
For instance, a new version may introduce new capabilities than the former, or it may have the same capabilities, but it could realize them better due to different capacities. 
...
In that case, he is (hopefully) trained to learn about the company's products and their functioning. 
...
We evaluated the quality of our implementation using by assessing the clarity, expressive capability, and flexibility of our ontology with respect to some research questions, some competency questions, and some application scenarios. 
The relevance of our work is precisely the ability to answer questions \refRQ{RQ:RQ1}-\refRQ{RQ:RQ1.2} and \refCQ{CQ:first-function-capability}-\refCQ{CQ:last-tag-function}, among others, so that it could be able to support applications in most applied scenarios where functional reasoning is required. 
We highlight, though, that we delegate to further work the specialisation of our work to an application-level.\TODO{<--FC:check if ok.}
%\begin{figure}[t]
%\includegraphics{}
