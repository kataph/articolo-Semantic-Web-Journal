...
Though a vast amount of work to model functions has already been carried out, the literature has not settled on a shared and well-defined \myComment{methodology} approach due to the variety of concepts involved and the modeling tasks that functional descriptions should satisfy. 
...
%and are characterized differently, if at all.
...
Therefore, one is always confronted with the problem of how functions of components contribute to functions of the system they compose, so that functional decomposition is ubiquitous. %, which themselves push the system towards the desired state.
Functions are often distinguished from their realization (or manifestation). Since in engineering there exist standard ways to realize functions\footnote{For example, the use of gearboxes made in a certain way in order to increase or reduce angular velocity, and \myComment{. Another example:} a brushless electric motor \myComment{may be understood as the class subsuming all models of brushless electric motors, but it can also be recognized as a method by which electric energy can be converted to torque.} as a way to convert electric energy to torque.}, 
...
the term engineering method refers primarily to technological principles and theories on which an implementation relies.}, or solutions \cite{pahl_engineering_2007}, or `ways of functional achievement' \cite{kitamuraOntologicalModelDevice2006}. 
\myComment{, the last two terms taken from \cite{pahl_engineering_2007} and \cite{kitamuraOntologicalModelDevice2006} respectively.}   
...
\myComment{Instead} For another view, Chandrasekaran and Josephson classify six types of behaviours mainly depending on their time duration (instant, interval, or unspecified) and the values of the state variables during that time~\cite{chandrasekaranFunctionDeviceRepresentation2000}.
...
%It would take too much space to explain the characteristics of and the differences between these frameworks. 
...
On the other hand, the authors of \YAMATO deepen their view of function in a series of additional papers, among them \cite{kitamuraOntologicalModelDevice2006, kitamuraCharacterizingFunctionsBased2013, mizoguchiFunctionalOntologyArtifacts2009,mizoguchiUnifyingDefinitionArtifact2016}, in which they motivate and detail their claim that functions are roles played by behaviours, the latter being processes of a certain type (notice that \YAMATO \myComment{'s authors are same research group behind FBRL development} and FBRL were developed by the same research group).
...
%The debate itself is beyond the scope of this paper, we just mention that while \BFO maintains functions as a subclass of dispositions, Jansen and Röhl 
...
%behaviour can also be characterized by two kinds of behaviour
%variables.
...
Engineers create an artifact to realize a certain interaction between the artifact itself and elements of the environment. The behaviour of an artifact to be the way in which that artifact participates in that interaction (e.g. \quotes{[the car] rattled when it hit the curve} ~\cite{chandrasekaranFunctionDeviceRepresentation2000}). 
...
\begin{itemize}
  ...
  \item behaviour can \myComment{also} be a quality, that is, something inhering into the behaving entity. 
  \myComment{In this case some authors speak of} Examples are provided by Borgo et al.~\cite{borgoFormalOntologicalPerspective2009}, see also Vermaas or Gero and colleagues which talk of attributes or dispositions \cite{vermaasConceptualFrameworkJohn2007,geroCategorisingTechnologicalKnowledge2002}.
\end{itemize} 
...
While the role-based view introduces behaviour as a perdurant and fits naturally with the view of ontological function that we will investigate later, the ontological notion of behavior can be resumed to distinguish the behaviour of the doer from that of the operand, if necessary.
...
%and can coexist. One should be careful to separate them, e.g, calling general-behaviour the first and engineering-behaviour the latter. Since we are only using the second notion in this paper, we will use the term behaviour simpliciter.
...
\myComment{\item[\mydf{functionOf}] $ \FunctionSysOf{x}{y} \myiff (\DOLCERole{x} \land \exists b,t  (\DOLCECLby{b}{x}{t}) \land 
  ...
  \exists z,g,b,t (\System{z} \land \goalOf{g}{z}  \land  \DOLCECLby{b}{x}{t} \land \DOLCEPart{y}{z}{t} \\ \land
    \forall b',t' (\myComment{(}\DOLCECLby{b'}{x}{t'} \myComment{\land \DOLCEPart{y}{z}{t'})} \myfi (\BehaviourConcrete{b'} \land 
      \participateAsDoer{y}{b'}{t'} \land \DOLCEPart{y}{z}{t'} \\ \hfill{} \land \causallyContr{b'}{g}))))  $ \TODO{S: mettendo $\DOLCEPart{y}{z}{t}$ nell'antecedente dell'implicazione allora $b$ non è più un evento necessariamente relativo a $y$ e potrebbe essere in altri momenti un evento che non è neppure un behaviour. non è quello che vogliamo, quindi dovremmo avere solo $\DOLCECLby{b'}{x}{t'}$ nell'antecedente. [FC: potrebbe essere che nella formula, come stava scritta prima, mancasse un "$\land$" tra la prima e la seconda riga, ma tranne quello mi sembra che la nuova formula sia equivalente alla vecchia][FC:ahhhhh, adesso ho capito cosa volevi dire! Sì, direi che bisogna rimuovere la seconda congiunzione. Ho rimosso.]}
...
Since functions perdurants, we can use this ontology-driven categorisation of perdurants to introduce a categorisation of functions.
...
\begin{figure}
  ...
  %, additionally, we do not have added terms related with information content. 
...
\bflist
  \item[\mydf{connect}] $ \Connect{x} \myiff (\DOLCECLbyBinary{y}{x} \myiff \DOLCEEvent{y} \land \exists a,b,c,s_i,s_f,t_i,t_f~(\sState{y}{s_i}{t_i} \land \eState{y}{s_f}{t_f} \land \Goal{s_f} \land \DOLCEPhysObj{a} \land \DOLCEPhysObj{b} \land \DOLCEPhysObj{c} \land \DOLCEPC{a}{s_i}{t_i} \land \DOLCEPC{a}{s_f}{t_f} \land \DOLCEPC{b}{s_i}{t_i} \land \DOLCEPC{b}{s_f}{t_f} \land %\exists c (
...
%%%
The type of all \ontoFunc{fullPlural} that involve some change in quality values (cf. qualitative events in~\cite{guarino} {\color{red}{[come sopra]}}) could be defined as follows:
\bflist
...
\begin{itemize}
  ...
  Therefore, events that maintain a stable temperature vs. events in which it oscillates cannot be distinguished by \ontoFunc{fullSingular} unless one enriches the function definitions by allowing further conditions to constrain the temperature value during the event (e.g., via variation patterns, see \cite{guarino} {\color{red}{[come sopra]}}).
  %In this particular case, one could also argue that the event function of the controller is to maintain the temperature in the oven around the target temperature\footnote{This could be expressed in the style of the previous definitions by comparing an initial state, in which the set value and the actual value are different, to a final state, in which said values are equal.}, while the oscillations of the temperature are merely a side effect of the chosen implementation method, and would be different, or even absent, if the implementation method was different. 
  %In general, though, if one actually wishes to classify a transformation that cannot be described by comparison of two states, then the method employed in the previous formulas is not sufficient and a suitable comparison of what happens {\em during} the event should be added. 
  This is in line with our view of \ontoFunc{fullPlural} but recall that one should consider only conditions that are expressible within the language of the ontology.\marginpar{\color{red}{aggiunto frase[FC:OK]}}
...
%%They then use these equations to analyze or predict how well the artifact would realize its functional requirements. I think if the authors could bridge these notions (behaviours, states, and modelling equations), it could be another important contribution of the paper. - cit. revisore 3
