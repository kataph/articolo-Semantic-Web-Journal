...
We also need to capture a different kind of founding relation, that we call \firstTimeKeyWord{definition-founding} ($\foundedDef{\cdot}{\cdot}$). We will use this relation with the meaning that entity \mytext{$x$ is definition-founded on some entity $y$ if and only if $y$ is used to define $x$}.
...
Since roles, as well as concepts, can be seen as reified classes, there exists a specialisation relation between roles, which we write as $\DOLCEConceptSubsum{\cdot}{\cdot}$:
\bflist
...
Finally, sometimes in the paper we will need to use a non-temporalized version of classification: constant classification, $\DOLCECLbyBinary{\cdot}{\cdot}$,
\bflist
...
\bflist
  ...
  \item[\myax{goalSubsum}] $ \Goal{x} \myfi \DOLCEDescription{x} \land \forall y,t (\DOLCECLby{y}{x}{t} \to \DOLCEStative{y})$
  \item \mytext{a goal is a description which classifies stative perdurants only}\TODOinline{[SB: ho esteso l'assioma, controlla][FC: se in DOLCE è ok che anche le descrizioni (che sono sussunte in NASO e non in CN), e non solo i concetti, classifichino, allora è ok. In tal caso, la frase "is the classification relation
  between a concept and its instances at a certain time", presente sopora, va cambiata. Se una descrizione descrive cose diverse (e.g. una descrizione di come implementare una funzione) cosa classifica? (eventualmente potrebbe doversi un po' modificare gli assiomi contenenti gli engineering methods)]}
\eflist
Examples of goals are \quotes{the temperature at the port B of the heat exchanger is between 80 and 110 Celsius degrees} and \quotes{the buzzer is clapping with a frequency of at least 10kHz}.
...
\bflist
  ...
  \TODOinline{\mbox{[SB: constant part è forse troppo forte?][FC: in effetti è un rimasuglio di quando non c'era l'argomento temporale. Adesso dovrebbe essere ok]}}
...
%\item[] \mytext{An ontological function generalizes some systemic function and is not contextualized by a system}
 ...
 \item \mytext{any ontological function is a description that constantly classifies systemic functions only}
...
That is, if we represent the functional decomposition of a `main'-function, say $\cst{f}$, into `sub'-functions, say $\cst{f}_1, \cst{f}_2, \dots, \cst{f}_n$, as $\decom(\cst{f};\cst{f}_1,\cst{f}_2,\dots,\cst{f}_n)$, then it does not seem possible to find a parthood relation such that $\decom$ reduces the mereological sum.
...
Precisely, we introduce roles $\mainFunctionRole{\cdot}$ and $\subFunctionRole{\cdot}$ \TODOinline{[SB: qual è il significato di queste relazioni? come fa un lettore a capire?][FC: basta che legga qua, poi legga avanti, poi torni indietro, faccia una piroetta, ci pensi un po', sappia il fatto suo, poi, dopo un po' ci arriva. Ristrutturato, aggiungendo esplicitamente i ruoli di main-function e sub-function]}, contextualized by a decomposition, such that:
\bflist
  \item[\myax{main-sub-functions-sussum}] $ (\mainFunctionRole{x} \lor \subFunctionRole{x}) \myfi (\DOLCERole{x} \land \exists \cst{m} \foundedROLE{x}{\cst{m}})$
  \item[] \mytext{main-functions and sub-functions are roles instantiation-founded on some engineering method}
\eflist 
Additionally, we assume that methods are always contexts for a main-function and some sub-functions:
\bflist
  \item[\myax{pre-method}] $ \Method{\cst{m}} \myfi \exists! \cst{main}, \cst{sub_1}, \dots, \cst{sub_n} (\mainFunctionRole{\cst{main}} \land \subFunctionRole{\cst{sub_1}} \land \dots \land \subFunctionRole{{\cst{sub_n}}} \land \foundedROLE{\cst{main}}{\cst{m}} \land \foundedROLE{\cst{sub_1}}{\cst{m}} \land \dots \land \foundedROLE{\cst{sub_n}}{\cst{m}}) $
  \item[] \mytext{for any engineering method of functional decomposition, say $\cst{m}$, there exist a main-function role and $n$ sub-function roles, where $n$ is some number depending on $\cst{m}$, which are instantiation-founded on $\cst{m}$ and are uniquely determined}
  \TODO{FC: qui ho assunto, come hai messo sopra, che le descrizioni-metodi possano instanziare. Se non è così va modificato (basterebbe e.g. sostituire la relazione di dipendenza da instantiational a o definition o semplice)}
\eflist
Since the main-function and the $n$ sub-functions roles of a given method are univoquely determined, we can represent the with functional symbols. In particular, we will write $\cst{main^m}$, $\cst{{sub}^m}_1$, \dots, $\cst{{sub}^m}_n$ to indicate the roles corresponding to the method $\cst{m}$, as per Axiom \refax{ax:pre-method}.
...
\bflist
  ...
  \item \mytext{$x$ is main-function-of a method $\cst{m}$ if and only if it is constantly classified by the main-function role correspondig to $\cst{m}$} 
  \item[\mydf{sub-function-of}] $ \subFunction{x}{\cst{m}} \myiff \DOLCECLbyBinary{\cst{f}}{\cst{sub}^{\cst{m}}_i} $
  \item \mytext{$x$ is main-function-of a method $\cst{m}$ if and only if it is constantly classified by the any of the sub-function roles correspondig to $\cst{m}$} 
\eflist}
Finally, the link between methods and decompositions is given in the following definition schema: 
\bflist
  \item[\mydf{method}] $\decom(\cst{f};\cst{f}_1,\cst{f}_2,\dots,\cst{f}_n) \myiff \\ ( \FunctionSys{\cst{f}} \land \FunctionSys{\cst{f}_1} \land \dots \land \FunctionSys{\cst{f}_n} \land \\ \exists \cst{m} (\Method{\cst{m}} \land \DOLCECLbyBinary{\cst{f}}{\cst{main}^{\cst{m}}} \land \DOLCECLbyBinary{\cst{f}_1}{\cst{sub}^{\cst{m}}_1} \land \ldots \land \DOLCECLbyBinary{\cst{f_n}}{\cst{sub}^{\cst{m}}_n}) $
 \item \mytext{$\cst{f}$ is decomposed in $\cst{f_1},\ldots,\cst{f_n}$ if and only if they are all systemic functions and there is a method with corresponding main-function $\cst{main}^{\cst{m}}$ and sub-functions $\cst{sub}^{\cst{m}}_1$, \dots, $\cst{sub}^{\cst{m}}_n$, which are constantly played by $\cst{f}$ and $\cst{f_1},\ldots,\cst{f_n}$, respectively}
\eflist
...
The intuition is that capabilities are relational qualities that are needed to carry out an ontological function, so that they refer to that function: in our terminology, they are definition-founded\TODOinline{[SB: qui c'è "definition-founded", prima c'era "definitional-founded", ricontrollare e unificare]} on an ontological function\footnote{In this context, a heuristic interpretation of definition-dependence at the meta-level could be similar to Definition \myex{ex:Capab}\TODOinline{[SB: nota che questo riferimento è saltato, a cosa ti riferivi?]}}. Note that they are not \refdf{def:founding}-founded, since an artifact could have a capability to function without actually functioning. Additionally, the function must be ontological and not systemic, because, otherwise, the bearing object would be associated \textit{a priori} with some given system. 
...
For example, the definition of a systemic function \refdf{def:function}-\refdf{def:functionOf}, the instantiation-founding definition \refdf{def:founding}, and the engineering method schema \refdf{def:method} cannot be expressed in \OWL. 
In these cases we replace the axioms with weaker versions, for instance, the definition of systemic functions is replaced with
\bflist
...
\end{align*}
\eflist
where $\mathtt{classifies}$ is the inverse relation of $\DOLCECLby{\cdot}{\cdot}{\cdot}$ (note that in this definition the fact that systemic function are founded on systems is preserved and highlighted).
...
The link between the temporalized and non-temporalized relation can be interpreted in different ways. For example, one could state that the non-temporalized relation holds if and only if the temporalized relation holds whenever one of the relation arguments exists, what argument precisely depends on the relation. This is our choice (only on a meta-level, since it cannot be implemented in \OWL) for the parthood \refdf{def:partConstant}, classification \refdf{def:CLnontemp}, and participation-like relations:
\bflist
...
One could also state that the non-temporalized relation holds if and only if the temporalized relation held true at some time:%. This is our choice for the participation relations, for example, for participation-as-doer: 
\bflist
\item[\myex{participationDoerNonTempBIS}] $ \participateAsDoerBinary{x}{y} \myiff \exists t \participateAsDoer{x}{y}{t}$
\eflist
There are also other possibilities and the choice of one of those over the others, which is done at the meta-level, must be planned carefully, since it changes the intended models for the \OWL ontology. For example, \refdf{def:participationDoerNonTemp} excludes partecipation-as-doer in, say, a chemical process only for its first part, while \refex{ex:participationDoerNonTempBIS} allows it, but in \OWL this difference is lost.
...
Moreover, in order to symplify the complex axiomatization of engineering methods and functions, we introduce in the \OWL ontology two binary relation, with the following semantic:
[copiare qui gli assiomi-->in corso.....]  
...
\end{comment}
or one could query what capacities of what components are  involved in executing the ontological functions present in a given system:
\begin{verbatim}
PREFIX : <http://www.co-ode.org/ontologies/ont.owl#>
...
\end{comment}
The previous queries showcase the possibility of linking capabilities to functions. This is important for one could develop, based on this link, an application allowing engineers to find components that can satisfy, and using what properties, a given functional requirement, e.g. during design.

The ontology also supports queries related to engineering methods and their relation with functions.
For example, the following query takes a given system as inputs and returns all systemic functions involved in a method, with their role (main-function or sub-function, but one could also specialize the ontology to be more expressive) and underlying component:
\begin{verbatim}
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
PREFIX : <http://www.co-ode.org/ontologies/ont.owl#>
...
Additionally, even though the \DOLCE ontology cannot be expressed in \OWL language, \OWL-adapted versions exists, such as \DOLCE-lite or \DOLCE-Ultralite\footnote{Both openly available at \url{http://www.loa.istc.cnr.it/dolce/overview.html}.}. Any of these could be used to align the ontology developed in this paper with \DOLCE. In our case, we used the \OWL version of \DOLCE that has been recently submitted to the ISO 21838 standard\footnote{\url{https://www.iso.org/standard/71954.html}.}, which can be found on GitHub\footnote{\url{https://github.com/kataph/function-method-ontology.git}.}, together with the ontology described in this section.
